...
export class BoardGameGeekAPI extends APIModel {
	...
	async searchByTitle(title: string): Promise<MediaTypeModel[]> {
		...
		for (const boardgame of Array.from(
			response.querySelectorAll("boardgame"),
		)) {
			const id = boardgame.attributes.getNamedItem("objectid")?.value;
			const title =
				boardgame.querySelector("name[primary=true]")?.textContent ??
				boardgame.querySelector("name")?.textContent ??
				undefined;
	...
	async getById(id: string): Promise<MediaTypeModel> {
		...
		const onlineRating = Number.parseFloat(
			boardgame.querySelector("statistics ratings average")?.textContent ?? "0",
		);
		const genres = Array.from(boardgame.querySelectorAll("boardgamecategory"))
			.map((n) => n.textContent)
			.filter((n) => n !== null);
		const complexityRating = Number.parseFloat(
			boardgame.querySelector("averageweight")?.textContent ?? "0",
		);
		const minPlayers = Number.parseFloat(
			boardgame.querySelector("minplayers")?.textContent ?? "0",
		);
		const maxPlayers = Number.parseFloat(
			boardgame.querySelector("maxplayers")?.textContent ?? "0",
		);
		const playtime = `${boardgame.querySelector("playingtime")?.textContent ?? "unknown"} minutes`;
		const publishers = Array.from(
			boardgame.querySelectorAll("boardgamepublisher"),
		)
			.map((n) => n.textContent)
			.filter((n) => n !== null);
